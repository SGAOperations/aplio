...
- **PR:** title `#XXX Ticket Name In Title Case`; **base branch `dev`** (never open feature PRs against `main`); body contains `Closes #XXX`; assign `b-at-neu`. `dev → main` is promoted only by `/release`.
