...
- **Failure / edge**
  ...
  - A draft shows "—" for the applied date, **Continue** as its primary action and **Delete** as its row action. On a position no longer accepting, **Continue** stays in place but is disabled, with the reason beside it (Deadline passed / Position closed / Not open yet, from `getApplicantActionBlockedReason`); **Delete** remains.
  ...
  - A withdrawn row shows **Edit & resubmit**, disabled with the same reason treatment once the window has closed; it has no row action, and its deadline tiers exactly as a draft's does.
...
- **Failure / edge**
  - The window closed while it was withdrawn → the row shows **Edit & resubmit** disabled with its reason beside it, rather than the button; the apply page (if reached directly) renders "Applications are closed" ("This position stopped accepting applications, so this application can no longer be edited or submitted.").
