...
- **Failure / edge**
  ...
  - `draft` and `withdrawn` render the header's note copy plus `⋯` instead of a move control; `accepted`/`rejected` render the terminal-decision note plus `⋯` — neither renders the split button.
...
- **Trigger** — the `⋯` button in the detail page header ("Status history and override for <name>"), or **See more** at the end of a table row's `⋯` menu ([PM-8](#pm-8-work-the-application-queue)).
