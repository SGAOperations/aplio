...
- **Happy path** — `requireManagerOrAdminOr404` gates the role (the `(auth)` layout only gates profile completeness). Query params are parsed with `.catch(undefined)` per field, so one malformed param never sinks the rest. The toolbar offers a position filter ("All positions"), an applicant filter ("All applicants"), a status filter ("All statuses", now including "Draft"), a debounced search over "Name, email, position, or date", **Clear filters**, and sortable columns (date, name, status). Results are scoped by `buildApplicationWhere(user, 'listable')` — a manager sees only their positions' applications; drafts are excluded from this default queue (withdrawn rows are kept). The position title in each row links to `/positions/[id]`. The applicant option list (`getReviewableApplicants`) is scoped the same way the results are, so a manager only ever sees applicants who applied to positions they manage. For the four unresolved statuses, each row's `⋯` opens the same `ApplicationStatusMenu` items as the detail page's caret ([PM-11](#pm-11-move-one-application-through-the-status-path)) with the next step as the first item instead of hoisted, ending in **See more** — no separator between the next step and Reject when the next step already is Accept (`reviewing`). `accepted`/`rejected` rows render no action control at all — the row returns `null`, mirroring the non-reviewable early return already in the same component; changing a final decision is detail-page only ([PM-14](#pm-14-override-a-status-undo-or-review-its-history)). Opening the dialog via **See more** calls the read-only `loadApplicationStatusHistory` action since the table has no pre-fetched history per row, showing the dialog's loading skeleton while the fetch is in flight.
  - **Drafts in progress.** A muted strip under the page header — hidden while the Draft filter is on, and while there are none — reads "N applications in progress" (singular at one) with a **View drafts** link that carries the active position/applicant/search filters. Picking **Draft** from the status filter (or **View drafts**) swaps the results region for `getDraftApplications` — identity and timestamps only, via `buildApplicationScopeWhere(user)` plus `status: 'draft'`, never `buildApplicationWhere`. Draft rows show the applicant's name and email (via the shared display helper — `applicantName` is null until submission), the position, "Started" (`createdAt`) and "Last updated" (`updatedAt`, sortable). Rows are plain text: no checkbox, no `⋯`, no link on the applicant name or the row itself, and the bulk bar never appears for them. Above the table, a muted line states the privacy boundary: "You can see who started an application, not what they've written. Draft answers stay private until the applicant submits." `/manage/applications/[id]` still 404s for a draft either way — the drafts view has no detail page.
- **Failure / edge**
