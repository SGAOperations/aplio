...
- **Trigger** — the `⋯` button in the detail page header ("Status history and override for <name>"), the header's **Change decision** button on a terminal status, or, on a table row ([PM-8](#pm-8-work-the-application-queue)), either **See more** at the end of the `⋯` menu or the row's own **Change decision** button on a terminal status.
