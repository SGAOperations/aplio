...
- **Happy path** — `PositionCreateDialog` → `createPosition`, guarded by `requireManagerOrAdmin`. Title is required; description defaults to empty so a draft can be created quickly; `opensAt`/`closesAt` are org-timezone day boundaries (`America/New_York`). A position can never be created `closed` — the Status options are role-derived (`getPositionCreateStatusOptions`): a manager sees only **Draft**, with a hint below the select explaining that only an admin can open a position; an admin sees **Draft** and **Open**. The creator is auto-connected as a manager so they can edit it immediately. Toast **"Position created"** and the dialog routes to the new position's edit page.
- **Failure / edge**
