...
- **Decision, single** (`application_accepted` / `application_rejected`) — from `updateApplicationStatus`, which covers the header's quick actions, the override Select, and the success toast's Undo action alike ([PM-11](#pm-11-move-one-application-through-the-status-graph), [PM-14](#pm-14-override-a-status-undo-or-review-its-history)). The send is **scheduled** `DECISION_EMAIL_DELAY_SECONDS` (10) seconds out via Resend's `scheduledAt`; the `EmailLog` row logs `scheduled` with the provider id. Any further status change to that application inside the window — a second override, a move-back, or the toast's Undo — cancels the pending send and marks the row `cancelled`. A cancel that fails (already dispatched) is logged with `error` and the row is left `scheduled` so the next status change retries it; critically, **that status change dispatches no email of its own** while the prior schedule's fate is unverified, since scheduling a second send on top of one that may still be live in Resend would double-email the applicant. A decision re-applied after a _successful_ cancel schedules a fresh send.
- **Decision, bulk** (`updateApplicationStatuses`) — sent **immediately** through Resend's batch endpoint, chunked at `RESEND_BATCH_MAX_EMAILS` (100) with permissive validation so one bad address can't sink the rest; each row logs `sent` with its own provider id. There is **no window** — bulk eligibility isn't forward-only, so a bulk move can land on a row that still holds a pending single-decision send; `dispatchBulkDecisionEmails` cancels it first, same as the single path, and skips any row whose cancel can't be verified. Available to every reviewer who can bulk-change status at all — there is no email-specific permission gate, so a manager's bulk accept/reject emails exactly as an admin's does ([PM-12](#pm-12-move-several-applications-at-once)).
...
- **End state** — the new status, plus one `ApplicationStatusEvent` row recording it; `/manage/applications` and the detail page are revalidated. `accepted`/`rejected` also remove the applicant's ability to withdraw ([AP-13](#ap-13-withdraw-an-application)). A decision email schedules 10 seconds out, cancellable by any further status change to the same application, including the toast's own Undo ([XC-9](#xc-9-applicant-email)).
