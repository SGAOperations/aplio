...
Two cron-triggered emails, both windowed by the **org calendar**, not by a per-manager "since last send" — daily and weekly are independent cadences that never gate each other.
...
- **Weekly** (`manager_weekly_digest`) — `GET /api/cron/manager-weekly-digest`, Vercel Cron at `0 13 * * 1` UTC, Mondays. Covers the previous Monday–Sunday org week: new applications, a status breakdown (`/manage/applications?status=<value>` per row), and the manager's currently-open positions as context. Sent when there's anything new **or** anything still unresolved — an open position with zero new applications and zero unresolved is not itself a reason to send.
