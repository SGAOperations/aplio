...
- **Composition** — everyone always gets **their own** non-draft applications on published positions, with public status only ([XC-8](#xc-8-applicant-facing-status-grouping)) — this covers a manager or admin who has also applied. Managers and admins additionally get a reviewer group: managed positions only for a manager, every published position for an admin, scoped the same way as [PM-8](#pm-8-work-the-application-queue)'s queue (`buildApplicationScopeWhere`). The reviewer group also carries "\<title\> was opened" / "\<title\> was reopened" items for positions in that same scope, sourced from `PositionStatusEvent` rows with `to: 'open'` (`getRecentPositionOpenings`), each linking to `/positions/[id]`; unlike applications, these get **no self-filter** — the admin who opened a position still sees it under "All positions". Both groups render when either has items.
- **Self-filter** — a row in the reviewer group for the viewer's own **application** is dropped; it already appears in "Your applications", and without the filter a manager or admin who applied to a position they review would see their own application twice. Position-opening rows are never self-filtered.
- **Newest 10, merged** — "Your applications" is capped at 10, newest `submittedAt` first. The reviewer group merges applications and position openings by timestamp and keeps the newest 10 **across both kinds** — an active feed can show fewer than 10 applications to make room for an opening, or vice versa. The application self-filter runs before the merge, so the reviewer group can show fewer than 10 when some of the newest application rows are the viewer's own.
