...
- **Happy path** — the card states "Deleting hides this position everywhere — the positions list, search results and any direct link. This can't be undone from the app." The confirmation names the position and, when relevant, adds "N unsubmitted draft applications will disappear too." `deletePosition` soft-deletes with the blocking condition folded into the `where`, so the check and the write are one atomic statement. Toast **"Position deleted"**. Deleting a position that was `open` or `closed` (not `draft`) is the only signal back to its managers — an unlinked "\<title\> was deleted" activity-panel item, derived from `deletedAt` ([XC-10](#xc-10-activity-panel)); a deleted draft produces no row.
- **Failure / edge**
