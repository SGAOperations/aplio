...
**Scope boundary.** Workflows say _what happens_. Authorization — _who may_ do a thing, and which guard enforces it — lives in the permissions documentation and in `.claude/docs/ENGINEERING.md` §3 (capability matrix). Entries here name the denial's user-visible shape, never restate the rule behind it.
...
- **Happy path** — `requireManagerOrAdminOr404` gates the role (the `(auth)` layout only gates profile completeness). Query params are parsed with `.catch(undefined)` per field, so one malformed param never sinks the rest. The toolbar offers a position filter ("All positions"), a status filter ("All statuses"), a debounced search over "Name, email, position, or date", **Clear filters**, and sortable columns (date, name, status). Results are scoped by `buildApplicationWhere(user, 'reviewable')` — a manager sees only their positions' applications; drafts and withdrawn rows are excluded. The position title in each row links to `/positions/[id]`.
- **Failure / edge**
