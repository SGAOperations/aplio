...
- **The current allowlist, as a checkable fact** — `gh *` / `git *` / `npm *` / `npx shadcn*`, the 18 read-only inspection commands (`grep`, `egrep`, `rg`, `find`, `ls`, `cat`, `wc`, `head`, `tail`, `sort`, `uniq`, `cut`, `diff`, `stat`, `file`, `echo`, `pwd`, `true` — bare or with arguments) and `mkdir -p`, plus `Edit(**)`/`Write(**)` for impl/revise. Anything else auto-denies. **Known gap — do not rely on `permissions.deny` to catch shell redirection:** `permissions.allow`/`deny` glob patterns match against parsed command tokens, not the raw shell string, so a pattern like `Bash(* > *)` cannot see (and cannot deny) `>`/`>>`/`| tee` — confirmed empirically; see the note under "Permission rationale" below and tracking issue **#326**. The **instruction** below ("write files with the Write/Edit tools") is the only thing currently preventing a read-only stage from writing via Bash redirection — there is no technical enforcement yet. Check `.claude/settings.json` itself if unsure about what _is_ enforced; don't guess from memory.
...
| `Bash(grep\|egrep\|rg\|find\|ls\|cat\|wc\|head\|tail\|sort\|uniq\|cut\|diff\|stat\|file\|echo\|pwd\|true)` (bare + with args) + `Bash(mkdir -p *)` | all stages          | Read-only shell inspection Read/Grep/Glob can't reach (piping, chaining, one-off counts), plus the `.temp/` scratch-dir step CLAUDE.md requires before every commit                                                                                                                                                |
...
**Known gap — shell redirection cannot be denied this way.** An earlier version of this section claimed a `permissions.deny` pattern (`Bash(* > *)` / `Bash(* >> *)` / `Bash(* | tee *)`) could stop the 18 read-only commands above from writing files via `>`/`>>`/`| tee`. Empirically, it cannot: `permissions.allow`/`deny` glob matching appears to operate on parsed command tokens (which is why an argument-based pattern like `find * -exec *` works), while shell redirection operators are consumed by the shell layer and never reach the string being compared — true for both a leading-wildcard pattern and one anchored on the command name (`Bash(echo * > *)` was tested directly and still let `echo hi > file` through). Closing this gap would need a `PreToolUse` hook that inspects the raw command string and returns a `permissionDecision: "deny"` — see the `hookSpecificOutput` schema in the Claude Code hooks reference — which `log-bash-denial.mjs` does not currently do (logging-only). Tracked as **#326**; until it lands, the "write files with the Write/Edit tools" rule above is a **convention agents are expected to follow, not a technical guarantee** — `plan-agent`'s and `review-agent`'s read-only status rests on them not doing this, not on the permission system stopping them.
