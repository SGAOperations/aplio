...
- **Fingerprint** — `Vercel` red while `run-prettier-check` / `run-linting-check` / `run-tsc-check` are green, on **two or more open PRs at once**. The status description reads `Deployment has failed — run this Vercel CLI command: npx vercel inspect … --logs` (that string, not a build log, is what agents can see; `vercel` is deny-listed). The build log opens with the branch-limit message from `scripts/vercel-preview-migrate.mjs`. A **single** PR red on its own is more likely a genuine build error, not quota. Note that in `gh pr view --json statusCheckRollup` the `Vercel` entry is a **StatusContext** (`.context` / `.state`) while the Actions checks are CheckRuns (`.name` / `.conclusion`) — read `(.name // .context)` and `(.conclusion // .state)`.
