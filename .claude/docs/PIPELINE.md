...
- **Fingerprint** — `Vercel` red while `run-prettier-check` / `run-linting-check` / `run-tsc-check` are green, on **two or more open PRs at once**. This cross-PR comparison is the **only** discriminator available to an agent, so use it: the check's own description is generic (`Deployment has failed — run this Vercel CLI command: npx vercel inspect … --logs`) and names neither Neon nor the quota, and `vercel` is deny-listed so its one instruction can't be followed. The explicit branch-limit message from `scripts/vercel-preview-migrate.mjs` exists **only in the Vercel build log, which is human-only** — never expect to read it. A **single** PR red on its own stays ambiguous (quota vs. a genuine build break); treat it as a build break. Note that in `gh pr view --json statusCheckRollup` the `Vercel` entry is a **StatusContext** (`.context` / `.state`) while the Actions checks are CheckRuns (`.name` / `.conclusion`) — read `(.name // .context)` and `(.conclusion // .state)`.
