...
- **Budget** — **10 branches total, 3 permanently held** (`dev`, production, `vercel-dev`), so **~7 PRs can hold a preview database at once**. In-flight PR count is bounded by the branch quota, not by agent capacity. Reclamation is automatic (branch auto-delete on merge plus Neon's sweep) — never a manual cleanup step.
