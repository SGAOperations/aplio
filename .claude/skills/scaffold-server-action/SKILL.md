...
6. **Wire-up & feedback** — the caller checks `if (result && 'error' in result)` → **specific error toast**; on success → **success toast** (and uses the returned record if any); an unexpected `throw` surfaces a **generic error toast** (`sonner`). Preserve input on failure. Pair with the form scaffold. **Throw vs. return `{ error }` — the decision test (§4):** _would you show this exact sentence to the user, and can they act on it?_ **yes → `return { error: '…' }`** (business-rule violations: "already applied", "position closed"); **no → `throw`** (auth/authorization, a should-exist record missing, DB/3rd-party, internal) — never return a message that shouldn't be shown.
